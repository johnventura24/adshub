...
<body>
    ...
    app.post('/api/admin/sync-dashboard', 
        authMiddleware.requireAuth,
        requireAdmin,
        async (req, res) => {
            try {
                console.log('🔄 Dashboard sync requested');
                
                // This endpoint triggers a refresh of dashboard data
                // The dashboard will automatically refresh when this is called
                
                res.json({
                    success: true,
                    message: 'Dashboard sync triggered',
                ...
                res.status(500).json({
                    success: false,
                    message: 'Failed to trigger dashboard sync: ' + error.message
                });
            }
        }
    );
  ...
  try {
    ...
    try {
      if (dataService.connections.supabase) {
        savedLeader = await dataService.saveLeadershipToSupabase(leaderData, req.user?.id);
        savedLeader = savedLeader ? savedLeader[0] : null;
      } else {
...
app.put('/api/leadership-team/:id', async (req, res) => {
  try {
    ...
    try {
      if (dataService.connections.supabase) {
        updatedLeader = await dataService.updateLeadershipInSupabase(leaderId, leaderData);
        updatedLeader = updatedLeader ? updatedLeader[0] : null;
      } else {
...
app.delete('/api/leadership-team/:id', async (req, res) => {
  try {
    ...
    try {
      if (dataService.connections.supabase) {
        deletedLeader = await dataService.deleteLeadershipFromSupabase(leaderId);
        deletedLeader = deletedLeader ? deletedLeader[0] : null;
      } else {
...
// ============================================================================
// FILTERED DATA ENDPOINTS FOR TEAM/USER FILTERING
// ============================================================================

// Get filtered rocks data
app.get('/api/filtered/rocks', 
    authMiddleware.requireAuth, 
    authMiddleware.requirePermission(authMiddleware.PERMISSIONS.READ),
    async (req, res) => {
        try {
            const { department, user } = req.query;
            console.log(`🎯 API: Fetching filtered rocks data (department: ${department}, user: ${user})`);
            const filteredData = await dataService.getFilteredRocksData(department, user);
            res.json({ success: true, data: filteredData });
        } catch (error) {
            console.error('❌ API: Error fetching filtered rocks data:', error);
            res.status(500).json({ 
                success: false, 
                message: 'Failed to fetch filtered rocks data',
                error: error.message 
            });
        }
    }
...
// Get filtered todos data
app.get('/api/filtered/todos', 
    authMiddleware.requireAuth, 
    authMiddleware.requirePermission(authMiddleware.PERMISSIONS.READ),
    async (req, res) => {
        try {
            const { department, user } = req.query;
            console.log(`📝 API: Fetching filtered todos data (department: ${department}, user: ${user})`);
            const filteredData = await dataService.getFilteredTodosData(department, user);
            res.json({ success: true, data: filteredData });
        } catch (error) {
            console.error('❌ API: Error fetching filtered todos data:', error);
            res.status(500).json({ 
                success: false, 
                message: 'Failed to fetch filtered todos data',
                error: error.message 
            });
        }
    }
...
// Get filtered issues data
app.get('/api/filtered/issues', 
    authMiddleware.requireAuth, 
    authMiddleware.requirePermission(authMiddleware.PERMISSIONS.READ),
    async (req, res) => {
        try {
            const { department, user } = req.query;
            console.log(`⚠️ API: Fetching filtered issues data (department: ${department}, user: ${user})`);
            const filteredData = await dataService.getFilteredIssuesData(department, user);
            res.json({ success: true, data: filteredData });
        } catch (error) {
            console.error('❌ API: Error fetching filtered issues data:', error);
            res.status(500).json({ 
                success: false, 
                message: 'Failed to fetch filtered issues data',
                error: error.message 
            });
        }
    }
...
// Get filtered scorecard data
app.get('/api/filtered/scorecard', 
    authMiddleware.requireAuth, 
    authMiddleware.requirePermission(authMiddleware.PERMISSIONS.READ),
    async (req, res) => {
        try {
            const { department, user } = req.query;
            console.log(`📈 API: Fetching filtered scorecard data (department: ${department}, user: ${user})`);
            const filteredData = await dataService.getFilteredScorecardData(department, user);
            res.json({ success: true, data: filteredData });
        } catch (error) {
            console.error('❌ API: Error fetching filtered scorecard data:', error);
            res.status(500).json({ 
                success: false, 
                message: 'Failed to fetch filtered scorecard data',
                error: error.message 
            });
        }
    }
...
app.post('/api/admin/ninety/upload', 
    ...
    async (req, res) => {
        try {
            const { dataType, department } = req.body;
            const file = req.file;
            ...
            try {
                ...
                if (dataService.connections.supabase) {
                    const result = await dataService.saveNinetyDataToSupabase(processedData, dataType, req.user.id);
                    savedCount = result ? result.length : 0;
                    console.log(`✅ Saved ${savedCount} ${dataType} records to Supabase`);
                } else if (isUsingFileBasedAuth()) {
    ...
    async (req, res) => {
        try {
            ...
            try {
                if (dataService.connections.supabase) {
                    // Load from Supabase
                    data = await loadNinetyDataFromSupabase();
                    console.log('✅ Loaded Ninety.io data from Supabase');
                } else if (isUsingFileBasedAuth()) {
                    // Fallback to JSON files
            ...
            // For admin panel, use admin scorecard data (Ninety.io uploads)
            // Dashboard will use Tableau data separately
            if (data.scorecard) {
                // Replace dashboard scorecard with admin scorecard data
                data.scorecard = await dataService.getAdminScorecardData();
            }
    ...
    async (req, res) => {
        try {
            ...
            if (success) {
                res.json({
                    success: true,
                    message: 'Department deactivated successfully (permanently preserved for data integrity)'
                });
            } else {
                res.status(404).json({
                    success: false,
...
function processNinetyExcelData(jsonData, dataType, department = 'General') {
    ...
    jsonData.forEach((row, index) => {
        ...
        switch (dataType) {
            case 'todos':
                // Extract user information from CSV data
                const assignedUser = row.Owner || row.owner || row.Assignee || row.assignee || row.ResponsiblePerson || row.responsible_person || row['Responsible Person'] || row.User || row.user || 'System';
            ...
            case 'rocks':
                // Extract user information from CSV data
                const assignedUserRocks = row.Owner || row.owner || row.Assignee || row.assignee || row.ResponsiblePerson || row.responsible_person || row['Responsible Person'] || row.User || row.user || 'System';
                
                processedData.push({
                    id,
                    title: row.Title || row.title || row.Rock || row.rock || row.Goal || row.goal || row.Objective || row.objective || `Rock ${index + 1}`,
                    description: row.Description || row.description || row.Notes || row.notes || row.Details || row.details || '',
                    owner: assignedUserRocks,
                    department: department,
                    assignedUser: assignedUserRocks,
                    status: row.Status || row.status || row.State || row.state || 'Open',
            ...
            case 'issues':
                // Extract user information from CSV data
                const assignedUserIssues = row.Owner || row.owner || row.Assignee || row.assignee || row.ResponsiblePerson || row.responsible_person || row['Responsible Person'] || row.User || row.user || 'System';
                
                processedData.push({
                    id,
                    title: row.Title || row.title || row.Issue || row.issue || row.Problem || row.problem || row.Description || `Issue ${index + 1}`,
                    description: row.Description || row.description || row.Notes || row.notes || row.Details || row.details || '',
                    owner: assignedUserIssues,
                    department: department,
                    assignedUser: assignedUserIssues,
                    status: row.Status || row.status || row.State || row.state || 'Open',
                    priority: row.Priority || row.priority || 'Medium',
            ...
            case 'scorecard':
                // Extract user information from CSV data
                const assignedUserScorecard = row.Owner || row.owner || row.Assignee || row.assignee || row.ResponsiblePerson || row.responsible_person || row['Responsible Person'] || row.User || row.user || 'System';
...
async function saveNinetyDataToFiles(processedData, dataType) {
    ...
    const backupPath = path.join(__dirname, 'data', `backup_${dataType}_${Date.now()}.json`);
    ...
    console.log(`✅ Saved ${processedData.length} ${dataType} records to persistent storage`);
    console.log(`📊 Total ${dataType} records: ${existingData.length}`);
...
async function loadDepartmentsFromFiles() {
    ...
    if (!fs.existsSync(filePath)) {
        console.log('📊 No departments file found, creating default departments...');
        // Create default departments if file doesn't exist
        const defaultDepartments = [
            {
                id: 'dept_default_1',
                name: 'General',
                description: 'Default department for general assignments',
                manager: 'admin',
                status: 'active',
                created_date: new Date().toISOString(),
                updated_date: new Date().toISOString()
        ...
        try {
            fs.writeFileSync(filePath, JSON.stringify(defaultDepartments, null, 2));
            console.log('✅ Created default departments file');
            return defaultDepartments;
        } catch (error) {
            console.error('Error creating default departments file:', error);
            return [];
        }
    }
    ...
    try {
        ...
        // Verify departments integrity
        if (!Array.isArray(departments)) {
            console.error('❌ Departments file corrupted, resetting to default');
            const defaultDepartments = [
                {
                    id: 'dept_default_1',
                    name: 'General',
        ...
        console.log(`✅ Loaded ${departments.length} departments from file`);
        return departments;
    } catch (error) {
...
async function saveDepartmentToFiles(department) {
    ...
    // Create backup before modifying data
    const backupPath = path.join(__dirname, 'data', `backup_departments_${Date.now()}.json`);
    ...
    if (fs.existsSync(filePath)) {
        try {
            ...
            // Create backup of existing data
            fs.writeFileSync(backupPath, fileContent);
            console.log(`📦 Created department backup: ${backupPath}`);
        } catch (error) {
    ...
    try {
        ...
        console.log(`✅ Saved department "${department.name}" to persistent storage`);
...
async function deleteDepartmentFromFiles(deptId) {
    // SAFEGUARD: Departments are permanent and should never be deleted
    console.log(`🚫 Department deletion blocked for ID: ${deptId}`);
    console.log('📊 Departments are permanent and cannot be deleted for data integrity');
    ...
    try {
        ...
        // Find the department and mark as inactive instead of deleting
        const deptIndex = departments.findIndex(dept => dept.id === deptId);
        if (deptIndex !== -1) {
            departments[deptIndex].status = 'inactive';
            departments[deptIndex].updated_date = new Date().toISOString();
            departments[deptIndex].deactivation_reason = 'Marked as inactive instead of deletion for data integrity';
            
            fs.writeFileSync(filePath, JSON.stringify(departments, null, 2));
            console.log(`✅ Department "${departments[deptIndex].name}" marked as inactive instead of deletion`);
            return true;
        }
...
async function loadNinetyDataFromSupabase() {
    ...
    try {
        // Load each data type from Supabase
        const [rocks, todos, issues, scorecard] = await Promise.all([
            dataService.getAdminUploadedData('rocks'),
            dataService.getAdminUploadedData('todos'),
            dataService.getAdminUploadedData('issues'),
            dataService.getAdminUploadedData('scorecard')
        ]);
        ...
        console.log(`✅ Loaded from Supabase - Rocks: ${data.rocks.length}, Todos: ${data.todos.length}, Issues: ${data.issues.length}, Scorecard: ${data.scorecard.length}`);
        
    } catch (error) {
        console.error('Error loading Ninety.io data from Supabase:', error);
    }
...
// Get Ninety.io data for main dashboard
app.get('/api/ninety-data/data',
    authMiddleware.requireAuth,
    authMiddleware.requirePermission(authMiddleware.PERMISSIONS.READ),
    async (req, res) => {
        try {
            console.log('✅ Main dashboard requesting data...');
            
            // Get data from admin uploads
            const data = await dataService.getAdminUploadedData('data');
            
            if (data && data.length > 0) {
                console.log(`✅ Found ${data.length} data records for main dashboard`);
                res.json({
                    success: true,
                    data: data,
                    count: data.length,
                    source: 'admin_uploads',
                    message: `Loaded ${data.length} data records from admin uploads`
                });
            } else {
                console.log('📋 No data records found for main dashboard');
                res.json({
                    success: true,
                    data: [],
                    count: 0,
            ...
            console.error('Error fetching data for main dashboard:', error);
            res.status(500).json({
                success: false,
...
// Dashboard sync endpoint for admin panel
app.post('/api/admin/sync-dashboard', 
    authMiddleware.requireAuth,
    requireAdmin,
    async (req, res) => {
        try {
            console.log('🔄 Dashboard sync requested from admin panel');
