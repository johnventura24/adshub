...
function initializeNinetyData() {
    ...
    // Load initial data from admin uploads
    loadNinetyData();
    
    // Force refresh data every 10 seconds to catch admin updates immediately
    setInterval(() => {
        console.log('🔄 Auto-refreshing data from admin uploads...');
        loadNinetyData();
    }, 10000);
    
    // Also refresh immediately when dashboard becomes visible
...
function setupNinetyEventListeners() {
    ...
    if (refreshNinetyBtn) {
        refreshNinetyBtn.addEventListener('click', () => {
            ...
            showNotification('Refreshing data from admin uploads...', 'info');
            loadNinetyData();
        });
    }
...
async function loadNinetyData() {
    try {
        ...
        console.log('✅ Ninety.io data loaded successfully from admin uploads');
        console.log('📊 Data counts - Rocks:', currentNinetyData.rocks?.length || 0, 
                   ...
                   'Data:', currentNinetyData.data?.length || 0);
...
async function loadNinetyTodos() {
    try {
        ...
        const url = `/api/filtered/todos${params.toString() ? '?' + params.toString() : ''}`;
        console.log('🌐 Fetching filtered todos from:', url);
...
async function loadNinetyRocks() {
    try {
        ...
        const url = `/api/filtered/rocks${params.toString() ? '?' + params.toString() : ''}`;
        console.log('🌐 Fetching filtered rocks from:', url);
...
async function loadNinetyIssues() {
    try {
        ...
        const url = `/api/filtered/issues${params.toString() ? '?' + params.toString() : ''}`;
        console.log('🌐 Fetching filtered issues from:', url);
...
// Debug function to check data state (can be called from browser console)
window.debugNinetyDataState = function() {
    console.log('🔍 Debugging Ninety.io data state...');
    console.log('📊 currentNinetyData:', currentNinetyData);
    console.log('📊 Data counts:');
    console.log('  - Rocks:', currentNinetyData.rocks?.length || 0);
    console.log('  - Todos:', currentNinetyData.todos?.length || 0);
    console.log('  - Issues:', currentNinetyData.issues?.length || 0);
    console.log('  - Data:', currentNinetyData.data?.length || 0);
    console.log('  - Scorecard:', Object.keys(currentNinetyData.scorecard || {}).length);
    
    // Test API endpoints
    console.log('🧪 Testing API endpoints...');
    fetch('/api/ninety-data/todos')
        .then(r => r.json())
        .then(data => console.log('✅ Todos API:', data))
        .catch(e => console.error('❌ Todos API error:', e));
    
    fetch('/api/ninety-data/rocks/raw')
        .then(r => r.json())
        .then(data => console.log('✅ Rocks API:', data))
        .catch(e => console.error('❌ Rocks API error:', e));
    
    fetch('/api/ninety-data/issues/raw')
        .then(r => r.json())
        .then(data => console.log('✅ Issues API:', data))
        .catch(e => console.error('❌ Issues API error:', e));
};
...
async function loadNinetyScorecard() {
    try {
        ...
        const url = `/api/filtered/scorecard${params.toString() ? '?' + params.toString() : ''}`;
        console.log('🌐 Fetching filtered scorecard from:', url);
...
async function loadSampleData() {
    ...
    try {
        // Load data from API endpoints (admin uploads)
        try {
            const todosResponse = await fetch('/api/ninety-data/todos');
            if (todosResponse.ok) {
                const result = await todosResponse.json();
                currentNinetyData.todos = result.data || [];
                console.log('✅ Loaded todos from admin uploads:', currentNinetyData.todos.length);
            }
        } catch (error) {
            console.error('Error loading todos:', error);
            currentNinetyData.todos = [];
        }
        
        try {
            const rocksResponse = await fetch('/api/ninety-data/rocks/raw');
            if (rocksResponse.ok) {
                const result = await rocksResponse.json();
                currentNinetyData.rocks = result.data || [];
                console.log('✅ Loaded rocks from admin uploads:', currentNinetyData.rocks.length);
            }
        } catch (error) {
            console.error('Error loading rocks:', error);
            currentNinetyData.rocks = [];
        }
        
        try {
            const issuesResponse = await fetch('/api/ninety-data/issues/raw');
            if (issuesResponse.ok) {
                const result = await issuesResponse.json();
                currentNinetyData.issues = result.data || [];
                console.log('✅ Loaded issues from admin uploads:', currentNinetyData.issues.length);
            }
        } catch (error) {
            console.error('Error loading issues:', error);
            currentNinetyData.issues = [];
        }
        
        try {
            const scorecardResponse = await fetch('/api/ninety-data/scorecard');
            if (scorecardResponse.ok) {
                const result = await scorecardResponse.json();
                currentNinetyData.scorecard = result.data || {};
                console.log('✅ Loaded scorecard from admin uploads');
            }
        } catch (error) {
            console.error('Error loading scorecard:', error);
            currentNinetyData.scorecard = {};
        }
        
        try {
            const dataResponse = await fetch('/api/ninety-data/data');
            if (dataResponse.ok) {
                const result = await dataResponse.json();
                currentNinetyData.data = result.data || [];
                console.log('✅ Loaded data from admin uploads:', currentNinetyData.data.length);
            }
        } catch (error) {
            console.error('Error loading data:', error);
...
function filterRocks() {
    const departmentFilter = document.getElementById('rocksTeamFilter').value;
    ...
    console.log('Filtering rocks:', { departmentFilter, statusFilter, archiveToggle });
...
function filterTodos() {
    const departmentFilter = document.getElementById('todosTeamFilter').value;
    const archiveToggle = document.getElementById('todosArchiveToggle').checked;
    
    console.log('Filtering todos:', { departmentFilter, archiveToggle });
    // TODO: Implement filtering logic
}
...
function filterIssues() {
    const departmentFilter = document.getElementById('issuesTeamFilter').value;
    const archiveToggle = document.getElementById('issuesArchiveToggle').checked;
    
    console.log('Filtering issues:', { departmentFilter, archiveToggle });
    // TODO: Implement filtering logic
}
...
// Filter functions for team and user filtering
async function applyFilters() {
    ...
    try {
        ...
        console.log('📊 Filter values:', { department: departmentFilter, user: userFilter });
        
        // Reload all Ninety.io data with filters
        await loadNinetyData();
        
        // Show filter status
        let filterStatus = 'All data';
        if (departmentFilter && userFilter) {
            filterStatus = `Department: ${departmentFilter}, User: ${userFilter}`;
        } else if (departmentFilter) {
            filterStatus = `Department: ${departmentFilter}`;
        } else if (userFilter) {
            filterStatus = `User: ${userFilter}`;
        }
        
        showNotification(`Filters applied: ${filterStatus}`, 'info');
...
// Populate filter dropdowns with available teams and users
async function populateFilterDropdowns() {
    try {
        console.log('🔄 Populating filter dropdowns...');
        
        // Get departments and users from the data
        const departments = new Set();
        const users = new Set();
        
        // Collect departments and users from all data types
        const dataTypes = ['todos', 'rocks', 'issues', 'scorecard'];
        
        for (const dataType of dataTypes) {
            try {
                const response = await fetch(`/api/ninety-data/${dataType}`);
                if (response.ok) {
                    const result = await response.json();
                    ...
                    data.forEach(item => {
                        ...
                        if (item.assignedUser) users.add(item.assignedUser);
                        if (item.owner) users.add(item.owner);
                    });
                }
            } catch (error) {
                console.warn(`Failed to load ${dataType} for filter population:`, error);
            }
        }
        ...
        console.log('✅ Filter dropdowns populated:', { departments: departments.size, users: users.size });
...
function populateRocksTable() {
    ...
    rocks.forEach((rock, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <span class="ninety-department-badge">${rock.department || 'General'}</span>
            </td>
            <td>
        ...
        if (teamTodos.length === 0) {
            ...
            teamTodos.forEach((todo, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <span class="ninety-department-badge">${todo.department || 'General'}</span>
                    </td>
        ...
        if (shortTermIssues.length === 0) {
            ...
            shortTermIssues.forEach((issue, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <span class="ninety-department-badge">${issue.department || 'General'}</span>
                    </td>
