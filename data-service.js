...
class DataService {
  ...
  async initialize() {
    ...
    if (isSupabaseConfigured()) {
      try {
        ...
        this.connections.supabase = supabase;
      } catch (error) {
        console.log('❌ Supabase connection failed:', error.message);
        console.log('⚠️ Falling back to legacy database connections...');
      }
    } else {
    ...
    if (this.config.postgres.user && this.config.postgres.database) {
      try {
        ...
        console.log('✅ PostgreSQL connection established (fallback)');
      } catch (error) {
        console.log('❌ PostgreSQL connection failed:', error.message);
      }
    }

    // Initialize MySQL connection if configured (fallback)
    if (this.config.mysql.user && this.config.mysql.database) {
      try {
        ...
        console.log('✅ MySQL connection established (fallback)');
      } catch (error) {
        console.log('❌ MySQL connection failed:', error.message);
      }
    }

    // Initialize MongoDB connection if configured (fallback)
    if (this.config.mongodb.database) {
      try {
        ...
        console.log('✅ MongoDB connection established (fallback)');
      } catch (error) {
  ...
  // Fetch scorecard data for DASHBOARD (from Ninety.io CSV uploads)
  async getScorecardData() {
    console.log('📈 Fetching dashboard scorecard data from Ninety.io uploads...');
    
    // First, try to get scorecard data from admin uploads (Ninety.io CSV files)
    try {
      const uploadedScorecard = await this.getAdminUploadedData('scorecard');
      if (uploadedScorecard && uploadedScorecard.length > 0) {
        console.log('✅ Scorecard data fetched from Ninety.io uploads:', uploadedScorecard.length, 'items');
        return this.transformScorecardData(uploadedScorecard);
      }
    } catch (error) {
      console.log('⚠️ Ninety.io scorecard data not available:', error.message);
    }
    ...
    // Fallback to sample data
    console.log('📋 Using fallback sample data for dashboard scorecard');
    return {
      customerSatisfaction: 92,
      teamEfficiency: 88,
      goalCompletion: 75,
      qualityScore: 94,
      source: 'ninety_io_fallback'
    };
  }

  // Transform Tableau scorecard data for dashboard display
  transformTableauScorecardForDashboard(tableauScorecard) {
    const today = new Date().toISOString().split('T')[0];
    
    return {
      customerSatisfaction: tableauScorecard.customerSatisfaction || 0,
      teamEfficiency: tableauScorecard.teamEfficiency || 0,
      goalCompletion: tableauScorecard.goalCompletion || 0,
      qualityScore: tableauScorecard.qualityScore || 0,
      source: tableauScorecard.source || 'tableau_public',
      lastUpdated: tableauScorecard.lastUpdated || new Date().toISOString(),
      recordCount: 1,
      // Add dashboard-specific properties
      extractionDate: today,
      dataSource: 'Tableau Public'
    };
  }

  // Fetch scorecard data for ADMIN PANEL (from Ninety.io CSV uploads)
  async getAdminScorecardData() {
    console.log('📈 Fetching admin scorecard data from Ninety.io uploads...');
    
    try {
      const uploadedScorecard = await this.getAdminUploadedData('scorecard');
      if (uploadedScorecard && uploadedScorecard.length > 0) {
        console.log('✅ Admin scorecard data fetched from Ninety.io uploads:', uploadedScorecard.length, 'items');
        return uploadedScorecard; // Return raw data for admin panel
      }
    } catch (error) {
      console.log('⚠️ Admin uploaded scorecard data not available:', error.message);
    }
    
    console.log('📋 No admin scorecard data found');
    return [];
  }
  ...
  async getAdminUploadedData(dataType) {
    ...
    try {
      ...
      if (this.connections.supabase) {
        try {
          ...
          if (data && data.length > 0) {
            console.log(`✅ Found ${data.length} ${dataType} records in Supabase`);
            return data;
          }
        } catch (supabaseError) {
      ...
      if (this.connections.postgres || this.connections.mysql || this.connections.mongodb) {
        try {
          ...
          if (result && result.rows && result.rows.length > 0) {
            console.log(`✅ Found ${result.rows.length} ${dataType} records in legacy database`);
            return result.rows;
          }
        } catch (dbError) {
          console.log(`⚠️ Legacy database query failed for ${dataType}:`, dbError.message);
        }
      }
      
      // Final fallback to JSON file (for migration period)
      ...
      try {
        ...
        if (data && data.length > 0) {
          console.log(`✅ Found ${data.length} ${dataType} records in file (migration fallback)`);
          return data;
        }
      } catch (fileError) {
  ...
  // Get raw rocks data for dashboard display (NEW)
  async getRocksData() {
    console.log('🎯 Fetching raw rocks data...');
    
    try {
      const uploadedRocks = await this.getAdminUploadedData('rocks');
      if (uploadedRocks && uploadedRocks.length > 0) {
        console.log('✅ Raw rocks data fetched from admin uploads:', uploadedRocks.length, 'items');
        return uploadedRocks;
      }
    } catch (error) {
      console.log('⚠️ Admin uploaded rocks data not available:', error.message);
    }
  ...
  // Get raw issues data for dashboard display (NEW)
  async getRawIssuesData() {
    console.log('⚠️ Fetching raw issues data...');
    
    try {
      const uploadedIssues = await this.getAdminUploadedData('issues');
      if (uploadedIssues && uploadedIssues.length > 0) {
        console.log('✅ Raw issues data fetched from admin uploads:', uploadedIssues.length, 'items');
  ...
  // Filter data by team and user
  filterDataByDepartmentAndUser(data, departmentFilter = null, userFilter = null) {
    ...
    return data.filter(item => {
      ...
      const userMatch = !userFilter || item.assignedUser === userFilter || item.owner === userFilter;
      return departmentMatch && userMatch;
    });
  }

  // Get filtered rocks data
  async getFilteredRocksData(teamFilter = null, userFilter = null) {
    console.log('🎯 Fetching filtered rocks data...');
    
    try {
      const uploadedRocks = await this.getAdminUploadedData('rocks');
      if (uploadedRocks && uploadedRocks.length > 0) {
        const filteredData = this.filterDataByDepartmentAndUser(uploadedRocks, teamFilter, userFilter);
        console.log(`✅ Filtered rocks data: ${filteredData.length} items (team: ${teamFilter}, user: ${userFilter})`);
        return filteredData;
      }
    } catch (error) {
      console.log('⚠️ Admin uploaded rocks data not available:', error.message);
  ...
  async getFilteredTodosData(teamFilter = null, userFilter = null) {
    console.log('📝 Fetching filtered todos data...');
    
    try {
      const uploadedTodos = await this.getAdminUploadedData('todos');
      if (uploadedTodos && uploadedTodos.length > 0) {
        const filteredData = this.filterDataByDepartmentAndUser(uploadedTodos, teamFilter, userFilter);
        console.log(`✅ Filtered todos data: ${filteredData.length} items (team: ${teamFilter}, user: ${userFilter})`);
        return filteredData;
      }
    } catch (error) {
      console.log('⚠️ Admin uploaded todos data not available:', error.message);
    }
  ...
  async getFilteredIssuesData(teamFilter = null, userFilter = null) {
    console.log('⚠️ Fetching filtered issues data...');
    
    try {
      const uploadedIssues = await this.getAdminUploadedData('issues');
      if (uploadedIssues && uploadedIssues.length > 0) {
        const filteredData = this.filterDataByDepartmentAndUser(uploadedIssues, teamFilter, userFilter);
        console.log(`✅ Filtered issues data: ${filteredData.length} items (team: ${teamFilter}, user: ${userFilter})`);
        return filteredData;
      }
    } catch (error) {
      console.log('⚠️ Admin uploaded issues data not available:', error.message);
  ...
  async getFilteredScorecardData(teamFilter = null, userFilter = null) {
    console.log('📈 Fetching filtered scorecard data...');
    
    try {
      const uploadedScorecard = await this.getAdminUploadedData('scorecard');
      if (uploadedScorecard && uploadedScorecard.length > 0) {
        const filteredData = this.filterDataByDepartmentAndUser(uploadedScorecard, teamFilter, userFilter);
        console.log(`✅ Filtered scorecard data: ${filteredData.length} items (team: ${teamFilter}, user: ${userFilter})`);
        return filteredData;
      }
    } catch (error) {
      console.log('⚠️ Admin uploaded scorecard data not available:', error.message);
  ...
  async saveToSupabase(tableName, data, createdBy = null) {
    if (!this.connections.supabase) {
    ...
    try {
      ...
      console.log(`✅ Successfully saved ${result.length} records to ${tableName}`);
      return result;
    } catch (error) {
  ...
  async updateInSupabase(tableName, id, updates) {
    ...
    try {
      ...
      console.log(`✅ Successfully updated record in ${tableName}`);
      return data;
    } catch (error) {
  ...
  // Save Ninety.io data to Supabase
  async saveNinetyDataToSupabase(data, dataType, createdBy = null) {
    const tableName = TABLES[`NINETY_${dataType.toUpperCase()}`];
    if (!tableName) {
      throw new Error(`Unknown data type: ${dataType}`);
    }

    return await this.saveToSupabase(tableName, data, createdBy);
  }

  // Save leadership team data to Supabase
  async saveLeadershipToSupabase(leaderData, createdBy = null) {
    return await this.saveToSupabase(TABLES.LEADERSHIP_TEAM, [leaderData], createdBy);
  }

  // Update leadership team data in Supabase
  async updateLeadershipInSupabase(leaderId, updates) {
    return await this.updateInSupabase(TABLES.LEADERSHIP_TEAM, leaderId, updates);
  }

  // Delete leadership team data from Supabase
  async deleteLeadershipFromSupabase(leaderId) {
    return await this.deleteFromSupabase(TABLES.LEADERSHIP_TEAM, leaderId);
  }

  // Log upload to Supabase
  async logUploadToSupabase(logData) {
    return await this.saveToSupabase(TABLES.UPLOAD_LOGS, [logData]);
  }

  // Get filtered data from Supabase
  async getFilteredDataFromSupabase(tableName, filters = {}) {
    if (!this.connections.supabase) {
      throw new Error('Supabase connection not available');
    }
    ...
    try {
      ...
      if (filters.assigned_user) {
        query = query.eq('assigned_user', filters.assigned_user);
      }
      if (filters.status) {
